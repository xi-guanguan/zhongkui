...
| 对象 | 层数 | 应用方式 | 视觉收益 |
|------|------|---------|---------|
| **铜钱** | 3~5 | 顶/底=方孔面，中间=边缘环 | 厚重金属感，不再扁平 |
| **钟馗** | 8~12 | 帽冠/身体分部件堆叠 | 角色有体积感，俯视角立体 |
| **5种鬼** | 4~8 | 每种鬼按体型堆叠（矮→高） | 鬼有"实体"感，不是纸片 |
| **奶茶杯** | 4~6 | 杯底→杯身→杯口→盖/吸管 | 商品图有吸引力 |
| **钩子头** | 3~4 | 金属钩的厚度感 | 钩子更有力量感 |
| **孟婆柜台** | 5~8 | 木质柜台截面堆叠 | 场景有纵深 |
| **灯笼** | 4~6 | 圆形灯笼从窄→宽→窄 | 孟婆店氛围道具 |
| **锁链环** | 2~3 | 链环截面堆叠 | 链条有金属厚度 |

### 分类策略

| 类别 | 对象 | 是否旋转 | 缓存策略 |
|------|------|---------|---------|
| **静态展示** | 奶茶杯、灯笼、柜台 | 否 | 预渲染1次，drawImage |
| **可旋转道具** | 铜钱、钩子头 | 是（8方向预渲染） | 8张合成图缓存 |
| **角色** | 钟馗、5种鬼 | 简化（左右翻转即可） | 左右各1张缓存 |
| **动态** | 锁链环 | 跟随链段方向 | 实时绘制（层少，开销小） |

### 旋转实现（预渲染8方向）

```javascript
// 预渲染 8 个旋转角度的合成结果
function prerenderRotations(layers, count) {
  var rotations = [];
  for (var r = 0; r < count; r++) {
    var angle = r * (Math.PI * 2 / count);
    ...
    ctx.rotate(angle);
    for (var i = 0; i < layers.length; i++) {
      ctx.drawImage(layers[i], -layers[i].width/2, -layers[i].height/2 - i);
    }
    rotations.push(off);
  }
  return rotations;
}
// 运行时: ctx.drawImage(rotations[dirIndex], x, y);
```
